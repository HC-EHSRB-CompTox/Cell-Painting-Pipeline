...
test_chem_well <- rbind(as.data.frame(list_all[[1]][4]), as.data.frame(list_all[[2]][4]))
rm(list_all)

#List of features
features <- as.data.frame(colnames(test_chem_well[-c(1:3)]))
...
sample_info <- as.data.frame(c("Well", "Chemical", "Concentration"))
...
test_chem_well <- dplyr::select(test_chem_well, -c(Concentration, Well))
