...
#Create a list of categories (ft_cats)
samples <- colnames(test_chem_well_cat)[2:ncol(test_chem_well_cat)]
ft_cats <- unique(cat_lev_data$features)

#Eliminate categories with no internal variance
test_chem_well <- lapply(ft_cats, function(x){
...
a <- tcpl_results[!is.na(tcpl_results$bmdu) & !is.na(tcpl_results$acc) & !is.na(tcpl_results$bmdl),]
