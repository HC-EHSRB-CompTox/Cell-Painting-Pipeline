#Category level Principal component analysis and Concentration-response analysis
...
samples <- colnames(test_chem_well_cat)[2:ncol(test_chem_well_cat)]
ft_cats <- unique(cat_lev_data$features)

pca_cat <- lapply(ft_cats, function(x){
  cat_dat <- test_chem_well[, grep(x, colnames(test_chem_well))]
...
#Plot Mahalanobis distances 
ggplot(mahal_dist_chem[mahal_dist_chem$chem[] == "Etoposide",], aes(x=concentration, y=mahal_dist, colour=feat)) +
  geom_point() +
  scale_y_continuous() +
...
#tcplfit2
tcpl_results <- lapply(chemicals, function(x){
...
split_col <- str_split_fixed(tcpl_results$name, "_", 2)
tcpl_results <- cbind("chem"=split_col[,1], "feat"=split_col[,2], tcpl_results[,-1])

#BMC filtering criteria
#Eliminate BMCs without ACC and BMU
a <- tcpl_results[!is.na(tcpl_results$bmdu) & !is.na(tcpl_results$acc) & !is.na(tcpl_results$bmdl) & !is.na(tcpl_results$ga),]

a <- a %>% filter(chem != "Sorbitol" & chem != "Staurosporine")
...
#Save tcpl results
write_csv(tcpl_results, file = "Category Mahalanobis - tcplResult.csv")
